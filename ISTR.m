...
function [Theta_final Gz_estm, Gc1, Gc2, Gcl] = ISTR( uc, Asys, Bsys, d, Ts, Am, A0 )
...
L1 = zeros(length(uc), 1);
L2 = zeros(length(uc), 1);
...
%     DBS_sys = zeros(length(X)+d, 1);
%     DBS_sys(d+1:length(X)+d) = X';
%     Acl = conv(A, R)+ DBS_sys';
%     Bcl = DBS_sys';
%     y_final(j) = outputestimation( Acl, Bcl/dc_gain, d, uc, y_final, j );
        
    % Calculation of the dc gain of the closed loop tf based on the estimated A, B, S, R, T
    X = conv(B, S);
    DBS = zeros(length(X)+d, 1);
    DBS(d+1:length(X)+d) = X';
    num_gain = sum(conv(T, B));
    den_gain = sum(conv(conv(A, R)+ DBS', B));
    dc_gain = num_gain/den_gain;
    
    
%     Acl = conv((conv((conv(A, R)+ DBS'), B)), R);
%     Bcl = conv(B, R)/dc_gain;
%     y_final(j) = outputestimation( Acl, Bcl, 2*d, uc, y_final, j );
    ...
    L1(j) = outputestimation( conv(R, B), T/dc_gain, d, uc, L1, j ); % L1 is calculated taking into consideration the dc gain in "T"
    L2(j) = outputestimation( R, S, 0, y_estm, L2, j );
    u(j)  = L1(j) - L2(j);

    % Filling the Phi matrix
    for i = 1 : na % this for loop udsed to fill parts in the same line that pertains to the output "denomenator"
        if ((j-i)<=0)
            Phi(j, i) = 0;
        else
            Phi(j, i) = -y_estm((j-i));
...
%Closed loop
Z  = conv(1, 1/dc_gain);
DBS_num = zeros(length(Z)+d, 1);
DBS_num(d+1:length(Z)+d) = Z';
Acl = alpha;
Bcl = DBS_num';
Gcl = tf(Bcl, Acl, Ts);
